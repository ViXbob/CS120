...
pub fn header_detect(
    ...
    for (index, &value) in data.iter().enumerate() {
        ...
        if now_correlation > power * 1.5
            && (now_correlation > max_correlation || ((now_correlation - max_correlation).abs() < 1e-7 && index > start_index))
            && now_correlation > 0.1
        {
            max_correlation = now_correlation;
            start_index = index;
...
pub fn frame_resolve(
    ...
    for i in 0..frame_length {
        buffer = data[(begin_index + i * sample_per_bit as usize)..(begin_index + (i + 1) * sample_per_bit as usize)].iter().map(|x : _| { Complex::<f32>::new(*x, 0.0) }).collect();
    ...
    Ok((result, (begin_index + frame_length * sample_per_bit as usize) as usize))
}
