...
use crate::ip::*;
use crate::physical::*;
use crate::ack::ack::{AckPackage, AckLayer, HEADER_LENGTH};
...
impl AckStateMachine {
    pub fn new(input_device: usize, output_device: usize, address: u8) -> Self {
        let physical_layer = PhysicalLayer::new_with_specific_device(BYTE_IN_FRAME, input_device, output_device);
    ...
    pub fn size_channel(&self) -> tokio::sync::mpsc::Receiver<usize> {
        let (sender, receiver) = tokio::sync::mpsc::channel(1);
        let offset = self.tx_offset.clone();
        std::thread::spawn(move || {
            let size = offset;
            loop {
                sender.blocking_send(size.load(Relaxed));
            }
        });
        receiver
    }
    pub async fn work(&mut self) {
        ...
        loop {
            ...
            self.state = match now_state {
                AckState::FrameDetection => {
                    if self.tx.is_empty() {
                        ...
                        packages = (0..WINDOW_SIZE).map(|index| {
                            let begin_index = (self.tx_offset.load(Relaxed) + index) * byte_in_frame;
                            if begin_index >= self.tx.len() { return None; }
                            Some(if begin_index + byte_in_frame >= self.tx.len() {
                                AckPackage::new(self.tx.iter().skip(begin_index).cloned().chain(padding::padding()).take(byte_in_frame), self.tx.len() - begin_index, self.tx_offset.load(Relaxed) + index, false, false, 0, 0)
                            } else {
                                AckPackage::new(self.tx.iter().skip(begin_index).take(byte_in_frame).cloned(), byte_in_frame, self.tx_offset.load(Relaxed) + index, true, false, self.address, 0)
                            })
                ...
                AckState::Tx(packages) => {
                    debug!("packages {:?} need to be sent!", (self.tx_offset.load(Relaxed) + 0..self.tx_offset.load(Relaxed) + WINDOW_SIZE));
                    trace!("package {} need to be sent!", self.tx_offset.load(Relaxed));
                    // trace!("{:?}", package.data);
                    ...
                    loop {
                        ...
                        let (new_ack, package_count) = async {
                            ...
                            let mut old_ack = self.tx_offset.load(Relaxed);
