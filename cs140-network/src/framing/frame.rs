...
pub fn generate_frame_sample_psk_from_bitvec(
    ...
    for (i, bit) in data.iter().enumerate() {
        for j in 1..(samples_per_bit as usize) + 1 {
            ...
            if *bit { rtn.push(value); } else { rtn.push(-value); }
        }
    }
    rtn
}

pub fn generate_frame_sample_test(data: &BitStore) -> Vec<f32> {
    let mut sample = Vec::with_capacity(data.len() * 8 + 256);
    let convert_bit_to_sample = |samples: &mut Vec<f32>, bit: bool| {
        if bit {
            samples.push(1.0);
            samples.push(1.0);
            samples.push(1.0);
